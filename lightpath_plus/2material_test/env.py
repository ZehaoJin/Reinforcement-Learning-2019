...
class Lightpath(object):
    ...
    man = {'x': 20., 'z': 50., 'l': 20.} #500*1000 display window
    state_dim = 3
    action_dim = 2
    ...
    def step(self, action):
        ...
        #take action
        action = np.clip(action, *self.action_bound)*5  #*3 so the bound is now [-3,3]
        ...
        #dt=dl/v
        if x<=50:
            dt=dl*self.index_of_refraction1
            #print('1')
        if x>50:
            dt=dl*self.index_of_refraction2
            #print('2')
        ...
        # state! current state is (x,z,t)
        s = np.concatenate((self.man_info,np.array([self.t])))
        self.man_info*=scale
