...
class ArmEnv(object):
    ...
    def step(self, action):
        ...
        #take action
        #action = np.concatenate((np.array([0.]),np.clip(action, *self.action_bound)*3))  #*3 so the bound is now [-3,3]
        action =np.clip(action, *self.action_bound)*3
        action[0]=0
        #print(action)
        ###print (dx,dz)
        #print(action)
